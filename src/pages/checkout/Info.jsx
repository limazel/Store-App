      ...
      <List>
        {cart?.cartItems.map((item) => (
          <ListItem key={item.product.productId} sx={{py:1, px:0}}>
            ...
            <ListItemText primary={item.product.title} secondary={`x${item.product.quantity}`}></ListItemText>
            <Typography variant="body1">{currencyTRY.format(item.product.price)}</Typography>
          </ListItem>
        ))}
      </List>
    </>
